...
编写日期：2026-09-22。内容基线：R06 实现提交 bf73ce7、结果提交 31a4116，以及对应的源代码、配置和实验归档。2026-09-24 增补：本地受控 CPU bundle 的持久发布/恢复、客户端推荐幂等键、商品管理与 Web 演示已实现；以下历史“待实现”段落需结合 [STATUS](STATUS.md) 阅读，不表示 R06 真实模型已在线接入。
...
当前系统由 **离线研究链路** 和 **本地服务演示** 两部分组成。离线链路已经完成采样、训练、推理、评价和结果审计；服务侧已有 PostgreSQL 会话、反馈、推荐结果、商品管理和单实例受控 CPU bundle 发布/恢复。未配置数据库时仍可使用进程内演示；真实 R06 模型尚未在线接入。
...
flowchart TB
  ...
  subgraph Service["本地服务：已实现"]
    API["FastAPI 业务与管理接口"]
    UseCase["Recommend 应用用例"]
    Rules["不可变快照、绑定校验、过滤与稳定排序"]
    Memory["无数据库时的进程内演示适配器"]
    DB["PostgreSQL 会话、请求、反馈、商品与发布记录"]
    Runtime["受控 CPU bundle 运行时"]
    Publish["单实例发布、接收屏障和重启恢复"]
    Web["本地推荐与管理页面"]
    API --> UseCase
    UseCase --> Rules
    Memory --> UseCase
  ...
  subgraph Planned["仍待实现"]
    Export["R06 冻结模型导出和线上适配"]
    Jobs["后台任务租约、多进程协调"]
    Public["公网身份与部署"]
  end
  Files -. "待导出" .-> Export
...
图中虚线表示尚未实现的连接。推荐接口调用 `Recommend` 业务用例；未配置数据库时使用进程内后端，配置 `EVOREC_DATABASE_URL` 后由 PostgreSQL 维护业务状态。已发布的受控 CPU 基线可以排序，但不代表真实 R06 研究模型已经上线；训练仍通过独立研究入口运行。
...
| 受控模型加载 | 已实现白名单 CPU 格式、资源上限、二次哈希和黄金样本回放 | [model_runtime.py](../src/evorec/infrastructure/model_runtime.py)；受控 CPU 格式可发布，R06 冻结权重仍未接入 |
| PostgreSQL、任务领取、模型发布 | 核心迁移、单实例受控发布与恢复已实现；后台任务租约和多进程协调待实现 | [正式迁移](../db/migrations/0002_m23_publication.sql)、[架构决策](architecture/decisions.md) |
| 前端、C++ 扩展、线上性能 | 本地零构建 Web 演示已实现；完整前端、C++ 与线上性能未验证 | [Web 说明](../web/README.md)、[C++ 边界](../cpp/README.md) |
...
本节包含已落地的首层 bundle 校验、可移植受控加载边界和后续服务接入设计。单实例受控 CPU bundle 发布协调与恢复已补齐；R06 真实模型适配器、持久后台任务执行者和多实例协调仍未实现。已有训练检查点、离线来源清单或通过受控加载，都不等于已经完成 R06 在线 bundle。
...
| 阶段 | 交付物 | 完成条件 |
| --- | --- | --- |
| M11/M12/M23 | PostgreSQL 会话、反馈、客户端推荐幂等及单实例受控发布已完成 | 相同推荐结果重放、冲突拒绝、发布切换和重启恢复在本机 PostgreSQL 验证 |
| M21/M22 | 首层 bundle 校验和白名单 CPU 受控加载已实现；真实冻结模型适配仍待实现 | 已拒绝路径、哈希、映射、向量、组件和黄金样本错配；后台过期执行者与真实模型加载失败隔离仍待验证 |
| 模型接入与展示 | 本地 Web 页面已覆盖推荐与管理；真实 R06 排序适配器和有限队列仍待实现 | 当前 CPU 基线只验证受控发布，不代表研究模型线上收益 |
