...
这一结果说明学习排序值得继续，但还没有结束：原 R05 运行的 ColdListMLP 只有一个种子；普通模型的标准差不提供它的稳定性证据。后续补充见下节。共享候选池也只覆盖 127 / 7,088 个冷目标，候选可达性仍是明确瓶颈。跨样本分数差、单种子收益和线上提升不能混为一谈。
...
2026-09-17 更新：冻结原 R05 的训练样例、文本编码器、候选池和所有超参数，重放 seed 17 并补 seed 29 / 43。原 seed 17 的权重、训练轨迹与测试排名全部复现，三个检查点只按验证 NDCG 选轮。

三个种子共训练 30 轮，最佳轮次为 3 / 8 / 7。冷目标 Top20 命中为 26 / 24 / 27，原 RRF 为 6；整体 NDCG@10 均值为 **0.009315 ± 0.000387**。这次标准差确实属于冷加权模型。

![冷加权三种子复验曲线](assets/evorec/r05-replication/learning-curves.png)

置信区间与种子波动回答的是不同问题。原测试 12,720 个请求来自 9,499 个用户，我们按用户整体配对抽样，保留请求加权口径；固定 10,000 次抽样，对三个人群、两个指标、两个基线与各个种子列出全部 48 项区间。

三个固定种子的逐请求指标平均相对 CF-blend 的整体 NDCG 差值为 **+0.002127，95% 边际区间 [+0.001341, +0.002953]**。这个指标平均不是集成排序，区间只针对固定检查点下的用户抽样，不包含训练随机性。

![用户聚类配对差值区间](assets/evorec/r05-replication/paired-intervals.png)

旧测试早已查看，48 项区间也没有进行多重比较校正。因此，本轮可补充稳定性与不确定性证据，不能升级为新的封闭测试或线上收益。候选池仍只覆盖 127 / 7,088 个冷目标，接下来优先研究多兴趣召回与排序适配。

实现、验证与结果分别 commit，开发分支为 codex/r05-cold-replication；训练绑定代码提交 7f065e2。完整分析复算得到相同 JSON 哈希。

[复验报告](../experiments/r05-cold-replication/report.md) · [全部区间](../experiments/r05-cold-replication/uncertainty.json) · [图片来源](assets/evorec/r05-replication/manifest.json)
...
展示时，功能、算法质量和服务性能分别提供证据。当前已有离线基线数据与训练曲线；R02 序列模型未超过强基线，R03 内容融合改善整体排序，R04 验证未支持替换原融合策略，R05 学习排序在新用户测试中获得同候选池收益；冷加权已补齐三种子复验，线上业务仍需独立验收。

## 十二、持续更新日志

更新日志按时间倒序追加。正文维护当前认识；如果后续证据改变旧结论，会保留修正日期和原因。

### 2026-09-17｜完成冷加权复验与用户聚类区间

**实际改动：** 重放 seed 17、补齐 seed 29 / 43，完成排名重算、48 项区间及完整分析重复验证；新增两张 PNG / SVG 配图。

**结果：** 冷目标命中为 26 / 24 / 27，整体 NDCG 均值 0.009315 ± 0.000387。结果在已查看的 R05 测试上获得，原模型选型保持不变。

**版本：** 协议、实现、结果分别提交；实现与结果在 codex/r05-cold-replication 分支保留，训练使用 7f065e2。

**下一步：** 登记 R06 多兴趣召回与排序适配的新留出协议。
