...
本文既包含已经落地的代码边界，也规定后续实现必须遵守的运行协议。PostgreSQL 会话、推荐、反馈及单实例受控 CPU bundle 发布/恢复、商品管理和本地 Web 页面已实现；真实 R06 模型、后台租约任务和多进程发布仍待实现，R01–R06 离线研究已完成。HTTP 层调用真实应用用例；配置数据库后会话、请求、结果和反馈可跨应用实例恢复，未配置时回退到进程内演示。进度与实测证据见 [STATUS](STATUS.md)。
...
研究链路为“原始数据 → 固定切分 → 样本与商品快照 → 基线/方法训练 → 候选与排序 → 统一评估 → 完成的运行记录”。目标逻辑模块为 data、models、retrieval、evaluation、export；当前 src/evorec/research/ 已实现完整流用户采样、统计基线、因果自注意力序列训练、训练限定内容编码、双塔与候选融合、验证选型、多种子评估和自动报告；research/ 保存配置与依赖记录。已保存研究检查点及重载证据；服务侧已有白名单 CPU bundle 受控加载边界，但研究检查点导出和真实在线模型适配仍待接入。
