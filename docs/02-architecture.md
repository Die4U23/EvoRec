...
本文既包含已经落地的代码边界，也规定后续实现必须遵守的运行协议。数据库、在线模型推理、后台发布和前端仍待实现；R01–R06 离线研究已完成。当前 HTTP 层除状态接口外，已接入进程内会话与推荐演示；它调用真实应用用例，但数据和结果在重启后丢失，不能视为 PostgreSQL 闭环。进度与实测证据见 [STATUS](STATUS.md)。
...
| --- | --- | --- |
| AdmissionPort.acquire | 验证访问与历史版本；持久记录接收；绑定会话与商品快照；获得和释放租约；终结失败请求 | 进程内适配器已接入；数据库适配器待实现 |
| RankingPort.rank | 使用指定 bundle；有限队列；可比候选分数；准确回传绑定与执行策略；取消与远端资源管理 | 演示热门排序已接入；模型适配器待实现 |
| ResultRecorderPort.save | 以 request_id 幂等保存商品位置、分数和完成状态，成功返回之前提交 | 进程内幂等保存已接入；事务适配器待实现 |
