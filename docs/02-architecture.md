...
本文既包含已经落地的代码边界，也规定后续实现必须遵守的运行协议。PostgreSQL 核心迁移及会话、推荐、反馈适配器已实现；在线模型推理、后台发布和前端仍待实现，R01–R06 离线研究已完成。HTTP 层调用真实应用用例；配置数据库后会话、请求、结果和反馈可跨应用实例恢复，未配置时回退到进程内演示。进度与实测证据见 [STATUS](STATUS.md)。
