...
本文既包含已经落地的代码边界，也规定后续实现必须遵守的运行协议。数据库、模型推理、后台发布和前端仍待实现。当前可运行的 HTTP 接口只有三个状态接口；推荐应用用例通过测试适配器检查，没有对外注册推荐接口。进度与实测证据见 [STATUS](STATUS.md)。
...
训练依赖与服务依赖分别锁定。当前环境没有安装 PyTorch、Faiss 或数据库客户端；内部 HTTP 服务与三个进程的启动编排也尚未实现。
...
研究链路为“原始数据 → 固定切分 → 样本与商品快照 → 基线/方法训练 → 候选与排序 → 统一评估 → 完成的运行记录”。目标逻辑模块为 data、models、retrieval、evaluation、export；当前 research/ 只有职责说明和实验配置模板，后续随基线逐个建立实现。
...
架构决策及重新评估条件见 [ADR 决策记录](architecture/decisions.md)。第一项后续工作仍是 R01 数据与基线可行性验证，同时可按 M11 接入业务持久化；先获得真实算法与数据证据，再扩展运行组件。
