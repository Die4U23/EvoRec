...
版本：0.2｜更新：2026-09-15｜阶段：M0 架构落地

**架构决定：采用模块化单体代码结构，围绕版本化产物连接离线研究和在线推荐；接入模型后，以业务 API、推理进程和后台任务进程隔离运行职责。** 首版面向个人开发与单机演示，先完成可复现实验和可追溯推荐，再根据测量决定扩展。

本文既包含已经落地的代码边界，也规定后续实现必须遵守的运行协议。数据库、在线模型推理、后台发布和前端仍待实现；R01 基线、R02 采样协议、离线 GPU 序列训练、首轮内容双塔融合、R04 冷商品保留 / 门控验证及 R05 神经排序训练已完成。当前可运行的 HTTP 接口只有三个状态接口；推荐应用用例通过测试适配器检查，没有对外注册推荐接口。进度与实测证据见 [STATUS](STATUS.md)。
...
训练依赖与服务依赖分别记录。研究环境已复用本机 PyTorch 2.9.0+cu126 并完成 GPU 训练，当前采用 system-site-packages，尚未验证干净环境复建；Faiss、数据库适配器、内部 HTTP 服务和三个进程的启动编排尚未接入。
...
│   ├── recommend.py       推荐应用用例，尚未注册 HTTP 路由
│   └── health.py          就绪查询用例
├── infrastructure/
│   └── readiness.py       当前未配置依赖的真实阻塞状态
├── api/app.py             三个状态路由及启动入口
...
| --- | --- | --- |
| AdmissionPort.acquire | 验证访问与历史版本；持久记录接收；绑定会话与商品快照；获得和释放租约；终结失败请求 | 接口与测试替身，数据库适配器待实现 |
| RankingPort.rank | 使用指定 bundle；有限队列；可比候选分数；准确回传绑定与执行策略；取消与远端资源管理 | 接口与测试替身，模型适配器待实现 |
| ResultRecorderPort.save | 以 request_id 幂等保存商品位置、分数和完成状态，成功返回之前提交 | 接口与测试替身，事务适配器待实现 |
| ReadinessPort.check | 检查数据库、运行时、活动版本和接收屏障 | 当前默认返回三项依赖未配置；真实探针待实现 |

方法签名见 [ports.py](../src/evorec/application/ports.py)。当前不会把测试替身注入公开应用。
...
当前自动检查使用测试替身证明应用层规则；不证明 PostgreSQL 事务、进程间发布、GPU 取消或 PRD 性能已经正确。具体测试数量和环境以 [最新验证记录](STATUS.md) 为准。

架构决策及重新评估条件见 [ADR 决策记录](architecture/decisions.md)。R01 可行性验证、R02 采样协议及第一轮序列训练已完成。R04 已完成冷商品保留与规则门控验证；R05 已实现模拟冷商品训练与候选内相关性学习；下一项研究工作是冷加权模型多种子复验与候选覆盖改善，同时可按 M11 接入业务持久化；以真实算法与数据证据决定后续运行组件。
