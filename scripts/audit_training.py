"""Independently audit stored experiment traces without using the model evaluator."""
import argparse
import csv
import gzip
import hashlib
...
def audit(run, output):
    ...
    for result in series["test_results"]:
        ...
        summaries.append({"name": result["name"], "rows": totals["all_positive_events"]["n"], "sha256": trace["sha256"]})
    ...
    neural = [row for row in series["test_results"] if "seed" in row]
    assert [row["seed"] for row in neural] == config["seeds"]
    assert all(row["checkpoint_reload_verified"] for row in neural)
    for key in metric_keys:
        values = [row["metrics"]["cohorts"]["all_positive_events"][key] for row in neural]
        saved = series["selected_neural_seed_summary"][key]
    ...
    report = {
        ...
        "traces": summaries,
        "limits": ["does not establish statistical significance or online performance",
